...
def ndfs(X, **kwargs):
    ...
    # build the graph Laplacian L = D - W.  The supplied version subtracted
    # a sparse matrix from a degree vector; using an explicit diagonal keeps
    # the intended n_samples x n_samples shape.
    W_dense = W.toarray() if hasattr(W, "toarray") else np.asarray(W)
    degree = np.asarray(W_dense.sum(axis=1)).reshape(-1)
    L = np.diag(degree) - W_dense

    max_iter = 1000
    obj = np.zeros(max_iter)
    for iter_step in range(max_iter):
        # update feature weights.  Algebraically this is the same as
        # inv(X.T @ X + beta*D) @ X.T @ F, but the Woodbury form avoids a
        # d x d inverse and is crucial for omics data where d >> n.
        d_diag = np.diag(D).copy()
        regularizer = beta * d_diag + 1e-6
        r_inv = 1.0 / np.maximum(regularizer, 1e-12)
        xr = X * r_inv[np.newaxis, :]
        middle = np.linalg.inv(I + np.dot(xr, X.transpose()))
        T_Xt = (r_inv[:, np.newaxis] * X.transpose()) @ middle
        W = T_Xt @ F
        ...
        # update M, where X @ T_Xt == X @ inv(...) @ X.T
        M = L + alpha * (I - X @ T_Xt)
        M = (M + M.transpose())/2
        # update F
...
def kmeans_initialization(X, n_clusters):
    ...
    kmeans = sklearn.cluster.KMeans(
        n_clusters=n_clusters, init='k-means++', n_init=10, max_iter=300,
